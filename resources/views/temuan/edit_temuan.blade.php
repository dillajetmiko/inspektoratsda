...
<li class="breadcrumb-item"><a href="/lhp">Temuan</a></li>
		...
		<form action="/temuan/update_temuan" method="post">
            ...
            Nomor LHP : 
                <select class="form-control" name="NOMOR_LHP">
                    @foreach ($NOMOR as $LHP)
                    @if ($LHP->NOMOR_LHP === $LHP]->NOMOR_LHP)
            ...
            Status  : <br>
                @if ($temuan[0]->status == 0)
                <label><input type="radio" name="STATUS" value="0" checked="checked"/> Belum Ditindak Lanjut </label>
                <label><input type="radio" name="STATUS" value="1" /> Belum Sesuai Rekomendasi </label><br>
                <label><input type="radio" name="STATUS" value="2" /> Sesuai Rekomendasi </label><br>
                @elseif ($temuan[0]->status == 1)
                <label><input type="radio" name="STATUS" value="0" /> Belum Ditindak Lanjut </label>
                <label><input type="radio" name="STATUS" value="1" checked="checked"/> Belum Sesuai Rekomendasi </label><br>
                <label><input type="radio" name="STATUS" value="2" /> Sesuai Rekomendasi </label><br>
                @elseif ($temuan[0]->status == 2)
                <label><input type="radio" name="STATUS" value="0" /> Belum Ditindak Lanjut </label>
                <label><input type="radio" name="STATUS" value="1" /> Belum Sesuai Rekomendasi </label><br>
                <label><input type="radio" name="STATUS" value="2" checked="checked"/> Sesuai Rekomendasi </label><br>
                @endif
                <br>
            ...
            Jenis Temuan : 
                <select class="form-control" name="KODE_JENIS_TEMUAN">
                    @foreach ($KODE as $TEMUAN)
                    @if ($LHP->KODE_JENIS_TEMUAN === $TEMUAN]->KODE_JENIS_TEMUAN)
                    <option value="{{ $TEMUAN->KODE_JENIS_TEMUAN}}" selected>{{ $TEMUAN->NAMA_JENIS_TEMUAN}}</option>
                    @else 
                    <option value="{{ $TEMUAN->KODE_JENIS_TEMUAN}}">{{ $TEMUAN->NAMA_JENIS_TEMUAN}}</option>
                    @endif
                    @endforeach
                </select> <br>
